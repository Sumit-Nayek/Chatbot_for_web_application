...
# Define default instructions for prompt engineering
...
if prompt := st.chat_input("How can I help you?"):
    ...
    try:
        completion = client.chat.completions.create(
            ...
            model="deepseek/deepseek-r1:free",  # Use the model you want
            messages=messages,
            max_tokens=200,  # Enforce response length limit
            temperature=0.7,  # Control creativity (0.7 for balanced responses)
        )
        ai_response = completion.choices[0].message.content
    except Exception as e:
        st.error(f"API request failed: {str(e)}")
